...
class AgendamentoSerializer(serializers.ModelSerializer):
    ...
    def validate_data_horario(self, value):
        ...
        if ((0 <= value.weekday() <= 4) and not ((time(hour=9) <= value.time() < time(hour=12)) or (time(hour=13) <= value.time() < time(hour=18)))) or (value.weekday() == 5 and not (time(hour=9) <= value.time() < time(hour=13))) or (value.weekday() == 6):
        ...
        if (value.minute % 30) != 0:
            raise serializers.ValidationError("Agendamentos devem ser de 30 em 30 minutos!")
        return value
    ...
    def validate_telefone_cliente(self, value):
        ...
        if resposta == None:
            raise serializers.ValidationError("Formato do telefone inválido !!! (+ppp(ddd)NNNNN-nnnn)")
        return value
    def validate(self, attrs):
        ...
        data_horario = datetime.fromisoformat(str(data_horario)).replace(tzinfo=pytz.UTC)
        telefone_cliente = attrs.get("telefone_cliente", "")
        email_cliente = attrs.get("email_cliente", "")
        # regra da aula para e-mail terminando em .br
        # tem que ter +55 no telefone
        if email_cliente.endswith(".br") and telefone_cliente.startswith("+") and not telefone_cliente.starswith("+55"):
            raise serializers.ValidationError("E-mail .br deve ter fone +55 !")
        # regras de validação para horários
        if data_horario:
            lst_email = Agendamento.objects.filter(email_cliente=email_cliente).filter(cancelado=False).dates("data_horario", 'day')
            # recusa agendamento caso e-mail já exista no dia
            # horários cancelados são desconsiderados
            if data_horario.date() in lst_email:
                raise serializers.ValidationError("Mesmo e-mail não pode agendar mais de uma vez por dia!")
            data_ini = data_fim = data_horario - timedelta(hours=data_horario.hour, minutes=data_horario.minute)
            data_fim += timedelta(hours=23, minutes=59)
            lst_horarios_dia = Agendamento.objects.filter(data_horario__gt=data_ini).filter(data_horario__lt=data_fim).filter(cancelado=False).datetimes("data_horario", "minute")
            # recusa caso horários indisponível
            # horários cancelados são desconsiderados
            if data_horario in lst_horarios_dia:
                raise serializers.ValidationError("Horário não está disponível!", code=200)
        return attrs
